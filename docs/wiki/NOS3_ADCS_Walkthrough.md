...
Then, as the ADCS app processes commands, it will filter out its own ground commands and telemetry requests, and then check for each of the telemetry packets from the afformentioned sensors. At this point, it will call ingest methods on those to parse out the data into its own structs, and update its own telemetry points. If you would like more information, look at the "generic_adcs_ingest.c" file. This file can be found in **/nos3/components/generic_adcs/fsw/cfs/src/generic_adcs_ingest.c**.

This data is then used internally by the ADAC (Attitude Determination and Attitude Control) method, which is called on the scheduler and parsed in the same function in app as mentioned above, which calls a method within **/nos3/components/generic_adcs/fsw/cfs/src/generic_adcs_adac.c**, which will load the injested data into internal structs, and then determine what mode ADCS is in. Based off of that, it will call the associated function, which will utilize that data to determine what actuator commands need to be sent out to achieve its desired result, and then will build up those commands.
