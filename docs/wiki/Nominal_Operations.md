...
## Anomalies
During, prior, or after a pass, anomalies may occur with the ground software, the antenna, or the spacecraft software.
Example anomalies may be that the ground software does not connect to the antenna, something is physically or electrically wrong with the antenna, the spacecraft radio does not respond to commanding, an experiment is inoperable, etc.
If the anomaly is minor, the spacecraft and antenna operators may be able to correct it in real time.
Otherwise, the spacecraft and antenna operators can take the issue offline and study the anomaly in more detail including bringing in more experienced personnel to guide them in formulating future pass plans to address the anomaly and to develop anomaly reports once the anomaly is resolved.
