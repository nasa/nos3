/*******************************************************************************
** File: newsim_device.c
**
** Purpose:
**   This file contains the source code for the NEWSIM device.
**
*******************************************************************************/
...
int32_t NEWSIM_ReadData(uart_info_t* device, uint8_t* read_data, uint8_t data_length)
{
    ...
    while((bytes_available < data_length) && (ms_timeout_counter < NEWSIM_CFG_MS_TIMEOUT))
    {
        ms_timeout_counter++;
        OS_TaskDelay(1);
        bytes_available = uart_bytes_available(device);
    ...
    if (ms_timeout_counter < NEWSIM_CFG_MS_TIMEOUT)
    {
        ...
        {
            #ifdef NEWSIM_CFG_DEBUG
                OS_printf("  NEWSIM_ReadData: Bytes read != to requested! \n");
            #endif
            status = OS_ERROR;
        } /* uart_read */
    }
...
*/
int32_t NEWSIM_CommandDevice(uart_info_t* device, uint8_t cmd_code, uint32_t payload)
{
    int32_t status = OS_SUCCESS;
    int32_t bytes = 0;
    uint8_t write_data[NEWSIM_DEVICE_CMD_SIZE];
    uint8_t read_data[NEWSIM_DEVICE_DATA_SIZE];

    /* Prepare command */
    write_data[0] = NEWSIM_DEVICE_HDR_0;
    write_data[1] = NEWSIM_DEVICE_HDR_1;
    write_data[2] = cmd_code;
    write_data[3] = payload >> 24;
    write_data[4] = payload >> 16;
    write_data[5] = payload >> 8;
    write_data[6] = payload;
    write_data[7] = NEWSIM_DEVICE_TRAILER_0;
    write_data[8] = NEWSIM_DEVICE_TRAILER_1;
    ...
    {
        /* Write data */
        bytes = uart_write_port(device, write_data, NEWSIM_DEVICE_CMD_SIZE);
        #ifdef NEWSIM_CFG_DEBUG
            OS_printf("  NEWSIM_CommandDevice[%d] = ", bytes);
            for (uint32_t i = 0; i < NEWSIM_DEVICE_CMD_SIZE; i++)
            {
                OS_printf("%02x", write_data[i]);
            }
            OS_printf("\n");
        #endif
        if (bytes == NEWSIM_DEVICE_CMD_SIZE)
        {
            status = NEWSIM_ReadData(device, read_data, NEWSIM_DEVICE_CMD_SIZE);
            if (status == OS_SUCCESS)
            {
                /* Confirm echoed response */
                bytes = 0;
                while ((bytes < (int32_t) NEWSIM_DEVICE_CMD_SIZE) && (status == OS_SUCCESS))
                {
            ...
            {
                #ifdef NEWSIM_CFG_DEBUG
                    OS_printf("NEWSIM_CommandDevice - NEWSIM_ReadData returned %d \n", status);
                #endif
            }
        } 
        else
        {
            #ifdef NEWSIM_CFG_DEBUG
                OS_printf("NEWSIM_CommandDevice - uart_write_port returned %d, expected %d \n", bytes, NEWSIM_DEVICE_CMD_SIZE);
            #endif
        } /* uart_write */
    } /* uart_flush*/
    return status;
...
{
    int32_t status = OS_SUCCESS;
    uint8_t read_data[NEWSIM_DEVICE_HK_SIZE];

    /* Command device to send HK */
    status = NEWSIM_CommandDevice(device, NEWSIM_DEVICE_REQ_HK_CMD, 0);
    if (status == OS_SUCCESS)
    {
        /* Read HK data */
        status = NEWSIM_ReadData(device, read_data, sizeof(read_data));
        if (status == OS_SUCCESS)
        {
            #ifdef NEWSIM_CFG_DEBUG
                OS_printf("  NEWSIM_RequestHK = ");
            ...
            if ((read_data[0]  == NEWSIM_DEVICE_HDR_0)     && 
                (read_data[1]  == NEWSIM_DEVICE_HDR_1)     && 
                (read_data[14] == NEWSIM_DEVICE_TRAILER_0) && 
                (read_data[15] == NEWSIM_DEVICE_TRAILER_1) )
            {
            ...
            {
                #ifdef NEWSIM_CFG_DEBUG
                    OS_printf("  NEWSIM_RequestHK: NEWSIM_ReadData reported error %d \n", status);
                #endif 
                status = OS_ERROR;
            }
        } /* NEWSIM_ReadData */
    ...
    {
        #ifdef NEWSIM_CFG_DEBUG
            OS_printf("  NEWSIM_RequestHK: NEWSIM_CommandDevice reported error %d \n", status);
        #endif 
    }
    return status;
}
...
int32_t NEWSIM_RequestData(uart_info_t* device, NEWSIM_Device_Data_tlm_t* data)
{
    ...
    /* Command device to send HK */
    status = NEWSIM_CommandDevice(device, NEWSIM_DEVICE_REQ_DATA_CMD, 0);
    if (status == OS_SUCCESS)
    {
        /* Read HK data */
        status = NEWSIM_ReadData(device, read_data, sizeof(read_data));
        if (status == OS_SUCCESS)
        {
            ...
            if ((read_data[0]  == NEWSIM_DEVICE_HDR_0)     && 
                (read_data[1]  == NEWSIM_DEVICE_HDR_1)     && 
                (read_data[12] == NEWSIM_DEVICE_TRAILER_0) && 
                (read_data[13] == NEWSIM_DEVICE_TRAILER_1) )
            {
                data->DeviceCounter  = read_data[2] << 24;
                data->DeviceCounter |= read_data[3] << 16;
                data->DeviceCounter |= read_data[4] << 8;
        ...
        {
            #ifdef NEWSIM_CFG_DEBUG
                OS_printf("  NEWSIM_RequestData: Invalid data read! \n");
            #endif 
            status = OS_ERROR;
        } /* NEWSIM_ReadData */
    }
    ...
    {
        #ifdef NEWSIM_CFG_DEBUG
            OS_printf("  NEWSIM_RequestData: NEWSIM_CommandDevice reported error %d \n", status);
        #endif 
    }
    return status;
}
